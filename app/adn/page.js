...
export default async function ADNPage() {
  ...
  const [{ data: progress }, { data: posts }] = await Promise.all([
    supabase.from("bsv_member_progress").select("*").eq("member_id", user.id).eq("completed", true),
    supabase.from("bsv_community_posts").select("*").order("created_at", { ascending: false }),
  ]);
